...
echo -e "${BLUE}==========================================================${PLAIN}"
echo -e "${BLUE}        欢迎使用 SijulyVPN 一键源码部署与管理脚本${PLAIN}"
echo -e "${BLUE}==========================================================${PLAIN}"

# 3. Configure GitHub Repository URL
# Default to the official repository (SIJULY/SijulyVPN)
DEFAULT_USER="SIJULY"
DEFAULT_REPO="SijulyVPN"
...
# 6. Configure global command shortcut "sj"
echo -e "\n${YELLOW}[4/4] 正在创建全局命令快捷接口 'sj'...${PLAIN}"
echo -e "  -> 正在写入管理脚本 /usr/bin/sj ..."
cat > /usr/bin/sj <<'EOF'
#!/usr/bin/env python3
import sys
import os
...
def print_status():
    ...
    print_line("=======================================================")
    print_line(f"               {bold}SijulyVPN 管理终端 v2.0{reset}                  ")
    print_line("=======================================================")
    print_line("【核心服务状态】")
...
def start_service():
    print("正在启动 SijulyVPN 服务...", flush=True)
    run_service_cmd("start")
    print("已发送启动指令。")
    time.sleep(1)

def stop_service():
    print("正在停止 SijulyVPN 服务...", flush=True)
    run_service_cmd("stop")
    print("已发送停止指令。")
    time.sleep(1)

def restart_service():
    print("正在重启 SijulyVPN 服务...", flush=True)
    run_service_cmd("restart")
    print("已发送重启指令。")
    time.sleep(1)

def show_logs():
    print("正在查看 SijulyVPN 日志 (按 Ctrl+C 退出)...", flush=True)
...
def uninstall_service():
    confirm = input("确定要完全卸载 SijulyVPN 吗？(y/N): ")
    if confirm.lower() == 'y':
...
def main():
    ...
    options = {
        '1': ("启动服务 (sj start)", start_service),
        '2': ("停止服务 (sj stop)", stop_service),
        '3': ("重启服务 (sj restart)", restart_service),
        '4': ("日志监控 (sj logs)", show_logs),
        '5': ("网页配置 (sj web)", configure_web),
        '6': ("端口配置 (sj port)", configure_port),
        '7': ("账号密码 (sj password)", configure_credentials),
        '8': ("一键更新 (sj update)", update_service),
        '9': ("完全卸载 (sj uninstall)", uninstall_service),
        '0': ("退出终端", None)
    }
...
echo -e "\n正在等待 SijulyVPN 首次获取节点并建立加密通道 (此过程可能需要 5-30 秒)..."
ACTIVE_ID=""
LAST_MSG=""
for i in {1..90}; do
...
echo -e "\n${GREEN}==========================================================${PLAIN}"
echo -e "${GREEN}             SijulyVPN 源码一键部署已完成！${PLAIN}"
echo -e "${GREEN}==========================================================${PLAIN}"
...
echo -e "  * 停止服务:       ${YELLOW}sj stop${PLAIN}"
echo -e "  * 重启服务:       ${YELLOW}sj restart${PLAIN}"
echo -e "=========================================================="
echo
